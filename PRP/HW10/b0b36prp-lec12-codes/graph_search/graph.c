...
_Bool load_bin(const char *fname, graph_t *graph)
{
   ...
   while (f && !feof(f) && reading) 
   {
      ...
      while (graph->num_edges < graph->capacity) 
      {
         int reads = 0;
         int buf = 0;
         reads += fread(&(buf), BIN_BUFFER, 1, f);
         graph->edges[graph->num_edges].from = htonl(buf);
         reads += fread(&(buf), BIN_BUFFER, 1, f);
         graph->edges[graph->num_edges].to = htonl(buf);
         reads += fread(&(buf), BIN_BUFFER, 1, f);
         graph->edges[graph->num_edges].cost = htonl(buf);
         if (reads < 3)
         {
            reading = 0;
            break;
...
_Bool save_bin(const graph_t * const graph, const char *fname)
{
   ...
   for (int i = 0; i < graph->num_edges; i++)
   {
      int num = ntohl(graph->edges[i].from);
      fwrite(&num, BIN_BUFFER, 1, f);
      num = ntohl(graph->edges[i].to);
      fwrite(&num, BIN_BUFFER, 1, f);
      num = ntohl(graph->edges[i].cost);
      fwrite(&num, BIN_BUFFER, 1, f);
   }
   if (f) 
   {
